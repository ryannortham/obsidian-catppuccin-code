...
assert(
  /\.nav-footer,[\s\S]*?&:not\(\.is-mobile\) \.workspace-split\.mod-sidedock \.workspace-sidedock-vault-profile\s*\{[\s\S]*?border-top: 0;/.test(interfaceSource),
  "Sidebar footers must not draw a top divider",
);
assert(
